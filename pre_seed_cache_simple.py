#!/usr/bin/env python3
"""
Simplified pre-seeding script for manga series cache
Compatible with EC2 environment
"""
...
from manga_lookup import DeepSeekAPI, GoogleBooksAPI, VertexAIAPI
...
class CachePreSeeder:
    ...
    def search_series(self, series_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            series_info = self.deepseek_api.search_series(series_name)
            if series_info:
                return series_info

            # Fallback to Vertex AI
            series_info = self.vertex_api.search_series(series_name)
            if series_info:
                return series_info

            # Final fallback to Google Books
            series_info = self.google_books_api.search_series(series_name)
            return series_info
    ...
    def get_volumes(self, series_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            # Try DeepSeek API first
            volumes = self.deepseek_api.get_volumes(series_name)
            if volumes:
                return volumes

            # Fallback to Vertex AI
            volumes = self.vertex_api.get_volumes(series_name)
            if volumes:
                return volumes

            # Final fallback to Google Books
            volumes = self.google_books_api.get_volumes(series_name)
            return volumes
