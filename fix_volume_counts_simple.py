#!/usr/bin/env python3
"""
Fix volume count issue - update series cache with actual volume counts (simple version)
"""
import sys
import os
...
def fix_volume_counts_simple():
    """Update series cache with actual volume counts using only existing fields"""
    cache = BigQueryCache()

    print('🔧 Fixing volume counts (simple version)...')
    ...
    for series_name in series_list:
        try:
            ...
            # Update series info with correct volume count using direct SQL update
            update_query = f"""
