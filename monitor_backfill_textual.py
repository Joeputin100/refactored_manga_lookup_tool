#!/usr/bin/env python3
"""
Textual TUI Monitor for BigQuery Metadata Backfill
...
class ProgressWidget(Static):
    """Widget for displaying progress with label and bar"""

    def __init__(self, label: str, progress: float, **kwargs):
        super().__init__(**kwargs)
        self.label = label
        self.progress = progress

    def compose(self) -> ComposeResult:
        yield Label(f"{self.label}: {self.progress:.1f}%")
        yield ProgressBar(total=100, progress=self.progress)

class RecentUpdatesWidget(Static):
    """Widget for displaying recent updates"""
    ...
    def watch_updates(self, updates: list) -> None:
        """Update when recent updates change"""
        self.update(self._render_updates(updates))
    ...
    def _render_updates(self, updates: list) -> str:
        """Render recent updates as formatted text"""
        if not updates:
        ...
        for vol in updates:
            ...
            lines.append(f"• {series} Vol {vol['volume_number']} [{fields_str}] {age_str} ago")
...
class StatsWidget(Static):
    ...
    def _render_stats(self, stats: dict) -> str:
        ...
        lines = [
            f"Total Volumes: {stats['total_volumes']}",
            f"Missing Descriptions: {stats['missing_descriptions']}",
            f"Missing ISBNs: {stats['missing_isbns']}",
            f"Missing Copyright Years: {stats['missing_copyright_years']}",
            f"Missing Publishers: {stats['missing_publishers']}",
            f"Missing Covers: {stats['missing_covers']}"
        ]
...
class BackfillMonitorApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            ...
            with Vertical(classes="progress-section"):
                ...
                yield ProgressWidget("📝 Descriptions", 0, id="desc-progress")
                yield ProgressWidget("📚 ISBNs", 0, id="isbn-progress")
                yield ProgressWidget("📅 Copyright Years", 0, id="year-progress")
                yield ProgressWidget("🏢 Publishers", 0, id="pub-progress")
                yield ProgressWidget("🖼️ Cover Images", 0, id="cover-progress")

            with Vertical(classes="stats-section"):
                yield StatsWidget(id="stats-widget")

            with Vertical(classes="updates-section"):
                yield RecentUpdatesWidget(id="updates-widget")
    ...
    def update_data(self) -> None:
        ...
        if 'error' not in stats:
            self.query_one("#overall-progress", ProgressBar).progress = stats['overall_progress']
            self.query_one("#desc-progress", ProgressWidget).progress = stats['descriptions_progress']
            self.query_one("#isbn-progress", ProgressWidget).progress = stats['isbns_progress']
            self.query_one("#year-progress", ProgressWidget).progress = stats['copyright_progress']
            self.query_one("#pub-progress", ProgressWidget).progress = stats['publishers_progress']
            self.query_one("#cover-progress", ProgressWidget).progress = stats['covers_progress']

        # Update stats and updates widgets
        self.query_one("#stats-widget", StatsWidget).stats = stats
        self.query_one("#updates-widget", RecentUpdatesWidget).updates = updates

        # Update footer with time
        self.query_one(Footer).update(f"Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S PST')} | Press Ctrl+C to exit")
