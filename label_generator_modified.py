#!/usr/bin/env python3
"""
Modified Label Generator for Manga Collection
Changes:
...
def draw_label(c, book, x, y, width, height):
    ...
    # Line 4: "Manga Collection" - MODIFIED to be two lines
...
def generate_sample_labels():
    ...
    # Generate PDF
    pdf_buffer = generate_pdf_labels(sample_books)

    # Save to file
    with open('manga_modified_labels.pdf', 'wb') as f:
        f.write(pdf_buffer.getvalue())

    return len(sample_books), pdf_buffer.getbuffer().nbytes

if __name__ == "__main__":
    print("🎯 Generating modified label PDF...")
    count, size = generate_sample_labels()
    print(f"✅ Generated modified label PDF: manga_modified_labels.pdf")
